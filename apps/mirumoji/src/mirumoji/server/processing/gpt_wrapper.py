...
class GptModel:
    ...
    MODEL_VERSIONS: ClassVar[tuple[str]] = (
        "gpt-4.1",
        "gpt-4o-mini",
        "gpt-4o",
        "gpt-4.1-mini",
    )
    FRMAP: ClassVar[dict[str, int]] = {
        "stop": 0,
        "length": 1,
        "function_call": 2,
        "content_filter": 3,
    ...
    FRCODEdict: ClassVar[dict[int, str]] = {
    ...
    def __init__(
        ...
        try:
            ...
            if version not in GptModel.MODEL_VERSIONS:
                LOGGER.error(
                    f"Model version provided is not supported, got"
                    f"'{version}'. Expected one of"
                    f" {GptModel.MODEL_VERSIONS}",
                )
                raise Exception(
                    f"Model version provided is not supported, got"
                    f"'{version}'. Expected one of"
            ...
            LOGGER.error(f"Error while cretaing Model Object : {e!s}")
            raise Exception(
                f"Error while cretaing Model Object : {e!s}",
            ) from e
    ...
    def _process_output(response: ChatCompletion) -> dict:
        ...
        try:
            ...
            raise Exception(
                f"Error converting token counts to int : {e}",
            ) from e
    ...
    def request(self, prompt: str) -> dict:
        ...
        try:
            ...
            LOGGER.error(f"Error Requesting : {e!s}")
            raise e
    ...
    def stream_request(self, prompt: str) -> Generator[Any, None, None]:
        ...
        try:
            ...
            raise Exception(
                f"Error during streaming request: {e}",
            ) from e
    ...
    def new_session(self) -> None:
        ...
        session_info = {
            "total_tokens": self.info["total_tokens"],
