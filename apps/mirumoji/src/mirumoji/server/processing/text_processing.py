...
from ..models.responses import BreakdownResponse
  ...
  Conclude with the specific nuance within the context sentence.
    ...
    def __init__(
        ...
        if not gpt_model_kwargs:
            gpt_model_kwargs = {}
        if "system_msg" not in gpt_model_kwargs:
            gpt_model_kwargs["system_msg"] = GptExplainService.SYSTEM_MSG
        if "version" not in gpt_model_kwargs:
            gpt_model_kwargs["version"] = version
        if "ApiKey" not in gpt_model_kwargs:
            gpt_model_kwargs["version"] = None
        if "from_dotenv" not in gpt_model_kwargs:
            gpt_model_kwargs["from_dotenv"] = True
        if "max_context" not in gpt_model_kwargs:
            gpt_model_kwargs["max_context"] = 100000

        self.model_kwargs = gpt_model_kwargs

    def explain(self, sentence: str, focus: str) -> str:
        """
...
class SentenceBreakdownService:
    ...
    def __init__(
        ...
        try:
            self.kb = Kotobase()
            self.kb.lookup("試し", wildcard=False, sentence_limit=1)
        except Exception as e:
    ...
    def single_word_lookup(self, word: str) -> DictLookup:
        ...
        default_jmentry = JMEntry(
            rank=99,
            kana=[],
            kanji=[],
            senses=default_sense,
        )
        default_jmnentry = JMNEntry(
            kana=[],
            kanji=[],
            translation_type="",
            gloss=[],
    ...
    def wildcard_lookup(self, pattern: str) -> DictWildcardLookup:
        """
    ...
    def explain_custom(
        ...
        # Generate GPT breakdown text
        if focus:
            gpt_text = self.gpt_explainer.explain_custom(
                sentence,
                focus,
                sysMsg,
                prompt,
            ...
            gpt_text = self.gpt_explainer.explain_sentence_custom(
                sentence,
                sysMsg,
                prompt,
                version,
