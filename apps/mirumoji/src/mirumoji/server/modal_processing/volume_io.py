...
tip: Additional Information
    ...
    - Each transfer is streamed in chunks and reported through a themed
...
def upload_to_volume(
    ...
    *,
    ...
    a themed transfer bar reports the progress
    ...
    Args:
        ...
        desc (str | None): Optional bar label. Defaults to
    ...
    try:
        with (
            src.open("rb") as raw,
            transfer_bar(desc, total=size) as advance,
            volume.batch_upload() as batch,
        ):
            # `CountingReader` is a transparent read-counting proxy that
            # delegates seek/tell/etc... to the real file, so it behaves like a
            # binary stream for the chunked upload while advancing the bar
            wrapped = CountingReader(raw, advance)
