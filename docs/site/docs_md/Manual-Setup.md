...
2.  **Create an Environment File for the API Keys:**
    ...
    -   **IMPORTANT:** If you choose to run without a local GPU, using [`MODAL`](https://modal.com), you'll need to insert your `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` into the `.env` file as well.
...
-   **GPU Issues (Backend):** If you encounter issues with the backend and GPU acceleration, double-check your NVIDIA driver installation and that the NVIDIA Container Toolkit is correctly set up. You can check Docker's access to the GPU by running this test image:
